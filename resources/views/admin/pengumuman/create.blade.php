...
@section('content')
    <div class="container mx-auto py-4 space-y-4">
        ...
        <div class="bg-gradient-to-r from-blue-200 to-cyan-400 rounded-2xl p-6 shadow-lg">
            <h1 class="text-2xl font-bold text-blue-900 mb-1">Create Announcement</h1>
            <p class="text-gray-700/80 text-sm">Fill in the form below to create a new announcement.</p>
        </div>
            ...
            <form method="POST" action="{{ route('admin.pengumuman.store') }}" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Maximum 5MB • PDF, DOC, DOCX, JPG, JPEG, PNG</p>
                    </div>

                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="status" value="1" id="status" {{ old('status') ? 'checked' : '' }}
                            class="w-4 h-4 text-blue-600 rounded">
                        <label for="status" class="text-sm font-medium text-gray-700">Publish Now</label>
                    </div>
                </div>
