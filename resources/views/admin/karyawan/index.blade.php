    ...
    <div class="container mx-auto py-4 space-y-4">
        ...
        <div class="bg-white rounded-2xl shadow p-4 md:p-6">
            <div class="grid grid-cols-1 sm:grid-cols-4 divide-y sm:divide-y-0 sm:divide-x">
                ...
                <!-- ITEM 4 -->
                <div class="flex flex-col gap-1 px-4 py-2">
                    <p class="text-gray-400 text-sm">Suspended/Terminated</p>
        ...
        <div class="bg-white rounded-2xl shadow p-4 md:p-6">
            ...
            <div class="overflow-x-auto">
                <table class="w-full min-w-[1100px] md:min-w-full text-sm text-left">
                    <thead>
                    ...
                    <tbody>
                        @foreach ($karyawans as $item)
                            ...
                            @if($item->role !== 'hr')
                            <tr class="employee-row border-b border-gray-100 hover:bg-blue-50/40 transition {{ $item->isSuspended() ? 'bg-red-50/30' : '' }}"
                                ...
                                <td class="py-3">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div
                                            class="w-9 h-9 rounded-full {{ $item->isSuspended() ? 'bg-red-100 border-red-200 opacity-60' : 'bg-blue-100 border-blue-200' }} overflow-hidden shrink-0">
                                            <img src="{{ $fotoUrl }}" alt="{{ $item->nama_lengkap }}"
                                                class="w-full h-full object-cover {{ $item->isSuspended() ? 'grayscale' : '' }}" loading="lazy"
                                                onerror="this.src='https://ui-avatars.com/api/?background=2563EB&color=fff&size=100&name={{ urlencode($item->nama_lengkap) }}'">
                                        </div>
                                        <div>
                                            <span class="font-medium text-gray-800 {{ $item->isSuspended() ? 'line-through text-gray-500' : '' }}">
                                                {{ $item->nama_lengkap }}
                                            </span>
                                            @if($item->isSuspended())
                                ...
                                <td class="py-3 text-gray-700 text-xs">
                                    @if(!$item->isActive() && $item->total_hari_kerja > 0)
                                        ...
                                        @if($item->end_date)
                                            <br>
                                            <span class="text-red-400 text-xs">End: {{ $item->end_date->format('d M Y') }}</span>
                                        @endif
                                ...
                                <td class="py-3">
                                    @if($item->isSuspended())
                                        <span class="inline-flex items-center gap-1 bg-red-100 text-red-700 py-1 px-2 rounded-full text-xs font-medium">
                                    ...
                                    @else
                                        <span class="inline-flex items-center gap-1 bg-green-100 text-green-700 py-1 px-2 rounded-full text-xs font-medium">
                                            <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
