<div id="announcement-modal-edit-{{ $item->id }}"
    ...
    <div class="relative bg-white w-full max-w-xl rounded-xl shadow-2xl overflow-hidden animate-fadeIn">
        ...
        <div class="flex items-center justify-between px-6 md:px-8 py-5 border-b">
            <div>
                ...
                <p class="text-gray-500 text-sm mt-1">
                    Update announcement details for employees
                </p>
            </div>

            <button data-modal-hide="announcement-modal-edit-{{ $item->id }}"
        ...
        <div class="max-h-[80vh] overflow-y-auto px-6 md:px-8">
            ...
            <form method="POST" action="{{ route('admin.pengumuman.update', $item->id) }}" enctype="multipart/form-data"
                class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="bg-gray-50 rounded-2xl border border-gray-100 p-5">
                    <div class="mb-5">
                        ...
                        <p class="text-sm text-gray-500">
                            Fill in the announcement details to be displayed
                        </p>
                    </div>
                ...
                <div class="bg-gray-50 rounded-2xl border border-gray-100 p-5">
                    ...
                    <div class="mb-5">
                        ...
                        <p class="text-sm text-gray-500">
                            Set category, target role, and publication schedule
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <select name="kategori"
                                ...
                                <option value="kebijakan"
                                    {{ old('kategori', $item->kategori) == 'kebijakan' ? 'selected' : '' }}>
                                    Policy
                                </option>
                                <option value="pengumuman"
                                    {{ old('kategori', $item->kategori) == 'pengumuman' ? 'selected' : '' }}>
                                    Announcement
                                </option>
                                <option value="event"
                                    {{ old('kategori', $item->kategori) == 'event' ? 'selected' : '' }}>
                                    Event
                                </option>
                                <option value="penting"
                                    {{ old('kategori', $item->kategori) == 'penting' ? 'selected' : '' }}>
                                    Important
                                </option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="target_role"
                                class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                <option value="all"
                                    {{ old('target_role', $item->target_role) == 'all' ? 'selected' : '' }}>
                                    All Role
                                </option>
                                <option value="admin"
                                    {{ old('target_role', $item->target_role) == 'admin' ? 'selected' : '' }}>
                                    Admin
                                </option>
                                <option value="hr"
                                    {{ old('target_role', $item->target_role) == 'hr' ? 'selected' : '' }}>
                                    HR
                                </option>
                                <option value="karyawan"
                                    {{ old('target_role', $item->target_role) == 'karyawan' ? 'selected' : '' }}>
                                    Employees
                                </option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="tanggal_terbit"
                                value="{{ old('tanggal_terbit', $item->tanggal_terbit ? $item->tanggal_terbit->format('Y-m-d\TH:i') : '') }}"
                                class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div class="flex items-center justify-between rounded-2xl border border-blue-100 bg-blue-50 px-5 py-4">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="status" value="1"
                            {{ old('status', $item->status) ? 'checked' : '' }} class="sr-only peer">
