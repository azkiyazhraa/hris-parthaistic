                    ...
                    <form id="editChangeDayForm" method="POST" class="space-y-5">
                        ...
                        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <p class="mb-2 text-xs text-gray-400">The regular work day (Mon–Sat) you want to take off
                                </p>
                            ...
                            <div>
                                ...
                                <p class="mb-2 text-xs text-gray-400">The Sunday or national holiday you'll work instead,
                                    within 1 week</p>
...
@push('scripts')
    <script>
            ...
            const fetchY = async (y) => {
                try {
                    ...
                    if (Array.isArray(data)) {
                        data.forEach(item => {
                            if (item.date) HOLIDAY_MAP_EDIT[item.date.substring(0, 10)] = item
        ...
        function isSundayEdit(str) {
            return parseLocalEdit(str).getDay() === 0;
        }

        function isHolidayEdit(str) {
            return Object.prototype.hasOwnProperty.call(HOLIDAY_MAP_EDIT, str);
        }
        ...
        document.getElementById('editOriginalDate').addEventListener('change', function() {
            ...
            if (isHolidayEdit(val)) {
                errEl.textContent =
                    `Original date cannot be a national holiday (${HOLIDAY_MAP_EDIT[val]}) — please select a regular work day.`;
                errEl.classList.remove('hidden');
                this.value = '';
                return;
            }
        ...
        document.getElementById('editChangeDayForm').addEventListener('submit', function(e) {
            ...
            if (!origVal || isSundayEdit(origVal) || isHolidayEdit(origVal)) {
                ...
                origErr.classList.remove('hidden');
                valid = false;
            }
